...
function App() {
  ...
  const [soundUris, setSoundUris] = useState<{ [key: string]: string } | null>(
    null
  );
  const [archiveView, setArchiveView] = useState<"active" | "archived">(
    "active"
  );
  const [selectedArchivedSpec, setSelectedArchivedSpec] = useState<
    string | null
  >(null);
  ...
  useEffect(() => {
    ...
    const unsubscribes = [
      ...
      vscodeApi.onMessage("tasks-updated", (message: any) => {
        ...
        if (message.data) {
          console.log(
            "Setting taskData with taskList count:",
            message.data.taskList?.length
          );
          console.log(
          ...
          if (!selectedSpec && message.data.specName) {
            console.log(
              "Setting selected spec from task data:",
              message.data.specName
            );
      ...
      vscodeApi.onMessage("approvals-updated", (message: any) => {
        ...
        console.log(
          "About to setApprovals - this should trigger badge counter update"
        );
      ...
      vscodeApi.onMessage("navigate-to-approvals", (message: any) => {
        console.log(
          "Navigating to approvals from native notification:",
          message.data
        );
  ...
  useEffect(() => {
    ...
    if (
      ...
      soundConfig.taskCompletionSound
    ) {
      console.log(
  ...
  const projectStats = React.useMemo(() => {
    ...
    const totalTasks = specs.reduce(
      (sum, spec) => sum + (spec.taskProgress?.total || 0),
      0
    );
                      ...
                      <div className="flex items-center space-x-2">
                        <div
                          className={cn(
                            ...
                            spec.taskProgress &&
                              spec.taskProgress.completed ===
                                spec.taskProgress.total &&
                              spec.taskProgress.total > 0
                              ? "bg-green-500"
                              : "bg-blue-500"
                          )}
            ...
            {selectedSpec ? (
                                  ...
                                  task.implementationDetails.length > 0 && (
                                    <div className="space-y-1">
                                      ...
                                      <ul className="text-xs text-muted-foreground list-disc list-inside space-y-0.5 ml-2">
                                        {task.implementationDetails.map(
                                          (detail, index) => (
                                            <li
                                              key={index}
              ...
              <div className="text-center text-muted-foreground text-sm py-8">
                {specs.length === 0
                  ...
                  : "Select a specification above to view tasks"}
              </div>
            )}
          </TabsContent>
            ...
            <div className="space-y-3">
              <div className="flex items-center space-x-2">
                ...
                >
                  ...
                  <SelectContent>
                    {specs.map((spec) => {
                      const specPendingCount = approvals.filter(
                        (approval) =>
                          approval.status === "pending" &&
                          approval.categoryName === spec.name
                      ).length;
            ...
            {selectedSpec ? (
              (() => {
                ...
                const pendingApprovals = approvals.filter(
                  (approval) =>
                    approval.status === "pending" &&
                    approval.categoryName === selectedSpec
                );
              ...
              <div className="text-center text-muted-foreground text-sm py-8">
                {specs.length === 0
                  ? "No specifications found"
                  : "Select a specification above to view pending approvals"}
              </div>
            )}
          </TabsContent>
              ...
              <div className="flex items-center space-x-2">
                ...
                {archiveView === "archived" && selectedArchivedSpec && (
                  <Button
                    ...
                    className="h-8 px-3 text-xs whitespace-nowrap"
                    onClick={() =>
                      vscodeApi.unarchiveSpec(selectedArchivedSpec)
                    }
                  >
                    Unarchive
                  </Button>
                  ...
                  <div className="text-center text-muted-foreground text-sm py-8">
                    {archiveView === "active"
                      ? specs.filter((spec) => !spec.isArchived).length === 0
                        ...
                        : "Select a specification above to view documents"
                      : archivedSpecs.length === 0
                      ? "No archived specifications found"
                      : "Select a specification above to view documents"}
                  </div>
                )}
              </CardContent>
                    ...
                    steeringDocuments.map((doc) => (
                      ...
                      >
                        ...
                        <Button
                          ...
                          disabled={!doc.exists}
                          onClick={() =>
                            vscodeApi.openSteeringDocument(doc.name)
                          }
                        >
                          Open
                        </Button>
